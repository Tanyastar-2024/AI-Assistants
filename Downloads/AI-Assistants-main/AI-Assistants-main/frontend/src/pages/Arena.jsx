...
export default function Arena() {
    ...
    const [bossDialogue, setBossDialogue] = useState("Loading Boss Arena...");
        ...
        const loadLectures = async () => {
            ...
            try {
                ...
                if (lectures.length > 0) {
                    ...
                    setBossDialogue("Choose your challenge!");
                } else {
                    setBossDialogue("No study materials found. Upload a lecture first!");
                }
            } catch (error) {
                console.error('Error loading lectures:', error);
                setBossDialogue("Error loading data.");
            }
        };
        loadLectures();
    }, [navigate]);
    ...
    const startGame = (lecture) => {
        ...
        setBossDialogue(`${questionsToPlay[0].question}`);
    };

    const processAnswer = (isCorrect, correctText) => {
        setIsLoading(true);
        setTimeout(async () => {
            if (isCorrect) {
                setIsSwinging(true);
                ...
                setTimeout(async () => {
                    ...
                    if (newHp === 0) {
                        ...
                        if (nextIndex < aiQuestions.length) {
                            ...
                            // Loop back if out of questions but boss is alive
                            setCurrentQIndex(0);
                            setBossDialogue(`${aiQuestions[0].question}`);
                        }
                ...
                setTimeout(() => {
                    ...
                    if (newHp === 0) {
                        ...
                        setBossDialogue(`WRONG! ${correctText}`);
    ...
    const handleTextAttack = async (e) => {
        e.preventDefault();
        ...
        processAnswer(isCorrect, `The correct answer was: ${currentQuestion.answer}`);
    };
        ...
        <div className={shouldShake ? "shake-screen" : ""} style={{ minHeight: '100vh', backgroundColor: '#0f172a', color: 'white', padding: '20px', overflow: 'hidden' }}>
            ...
            {!selectedLecture ? (
                <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
                    <h1 style={{ color: '#f43f5e', fontSize: '32px', marginBottom: '10px' }}>Choose Your Challenge</h1>
                    <p style={{ color: '#94a3b8', marginBottom: '30px' }}>Select a lecture to battle the boss using its flashcards.</p>
                    {availableLectures.length === 0 ? (
                        <div style={{ background: '#1e293b', padding: '30px', borderRadius: '12px' }}>
                            <p>Loading lectures or no lectures available...</p>
                        </div>
                    ) : (
                ...
                <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', maxWidth: '850px', margin: '0 auto 40px' }}>
                        ...
                        <div style={{ flex: 1, marginLeft: '15px', textAlign: 'right' }}>
                            <div style={{ fontSize: '12px', marginBottom: '5px', color: '#f43f5e', fontWeight: 'bold' }}>BOSS ({selectedLecture.title}): {bossHp}%</div>
                    ...
                    <div style={{ maxWidth: '600px', margin: '0 auto', position: 'relative', zIndex: 10 }}>
                        <form onSubmit={handleTextAttack} style={{ display: 'flex', gap: '10px' }}>
                            <input 
                        ...
                        <div className="game-over-screen">
                            <div className="victory-card">
                                ...
                                <div style={{ display: 'flex', gap: '15px', justifyContent: 'center' }}>
                                    <button onClick={() => setSelectedLecture(null)} className="btn-retry" style={{background: '#475569', marginTop: '20px'}}>Slay Another</button>
                                    <button onClick={() => navigate('/dashboard')} className="btn-retry" style={{background: '#3b82f6', marginTop: '20px'}}>Collect Rewards</button>
                                </div>
                            </div>
                        </div>
                    )}
            ...
            <style>{`
                ...
                .boss-bubble { position: absolute; top: 15px; left: 50%; transform: translateX(-50%); background: white; color: black; padding: 12px 20px; border-radius: 15px; font-weight: bold; width: 280px; text-align: center; font-size: 14px; box-shadow: 0 5px 20px rgba(0,0,0,0.4); z-index: 5; }
